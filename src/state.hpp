...
class State{
  ...
  aes_types::ilist slice(int start, int stop, size_t step) const {
  ...
  aes_types::ilist slice(int start, int stop) const {
    return this->slice(start, stop, 1);
  }
  aes_types::ilist slice(int stop) const {
  ...
  State shift_left(size_t rounds) const {
    aes_types::state_matrix_row rows = this->get_rows();
    aes_types::state_row row;
    aes_types::state_arr result;
    for (size_t row_idx = 0; row_idx < aes_constants::state_rows; row_idx++){
      row = aes_functions::shift_row_left(rows[row_idx], rounds);
      for (size_t column_idx = 0; column_idx < aes_constants::state_columns; column_idx++){
        result[column_idx*aes_constants::state_columns+row_idx] = row[column_idx];
      }
    }
    return State(result);
  }
  State shift_right(size_t rounds) const {
    ...
    for (size_t row_idx = 0; row_idx < aes_constants::state_rows; row_idx++){
      // calculate shift
      row = aes_functions::shift_row_right(rows[row_idx], rounds);
      // foreach column index
      for (size_t column_idx = 0; column_idx < aes_constants::state_columns; column_idx++){
        // flatten 2d index into 1d index
        size_t result_idx = column_idx*aes_constants::state_columns+row_idx; 
        result[result_idx] = row[column_idx];
      }
    }
    return State(result);
  }

  operator aes_types::ilist() {
    aes_types::ilist vct;
